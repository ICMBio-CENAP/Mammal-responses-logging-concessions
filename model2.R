...
sink(here("src", "model_foreco.txt"))
cat("
   model{
   ...
   mu.psi0 ~ dnorm(0, 0.1)	# intercept
   mu.psi1 ~ dnorm(0, 0.1)	# intensity
   mu.psi2 ~ dnorm(0, 0.1)	# road500

   # parameter related to detection
   mu.p0 ~ dnorm(0, 0.1) # intercept
   mu.p1 ~ dnorm(0, 0.1)  # intensity
   mu.p2 ~ dnorm(0, 0.1)  # road500
   mu.p3 ~ dnorm(0, 0.1)  # dates

   # precision
   tau.psi0 ~ dgamma(0.1, 0.1)
   tau.psi1 ~ dgamma(0.1, 0.1)
   tau.psi2 ~ dgamma(0.1, 0.1)

   tau.p0 ~ dgamma(0.1, 0.1)
   tau.p1 ~ dgamma(0.1, 0.1)
   tau.p2 ~ dgamma(0.1, 0.1)
   tau.p3 ~ dgamma(0.1, 0.1)
   ...
   a.psi1[i] ~ dnorm(mu.psi1, tau.psi1)  # intensity on psi
   a.psi2[i] ~ dnorm(mu.psi2, tau.psi2)  # road500 on psi

   a.p0[i] ~ dnorm(mu.p0, tau.p0)
   a.p1[i] ~ dnorm(mu.p1, tau.p1)  # intensity on p
   a.p2[i] ~ dnorm(mu.p2, tau.p2)  # roads on p
   a.p3[i] ~ dnorm(mu.p3, tau.p3)  # dates on p
   ...
   for (j in 1:J) {
     ...
     + a.psi1[i]*intensity500[j] + a.psi2[i]*road500[j]
   ...
   for (k in 1:K[j]) {
     ...
     + a.p1[i]*intensity500[j] + a.p2[i]*road500[j] + a.p3[i]*dates[j,k]
...
jags_data = list(n=n, nzeroes=nzeroes, J=J, K=effort, X=X, block=block, nblock=nblock, year=year, nyear=nyear,
                 intensity500=intensity500, road500=road500, dates=dates)

# Specify the parameters to be monitored
params = c("N", "Nsite",
              "a.psi0", "a.psi1", "a.psi2", 
              "a.p0", "a.p1", "a.p2", "a.p3",
...
inits = function() {
  ...
  list(omega = omegaGuess,
       ...
       a.psi0=rnorm(n+nzeroes), a.psi1=rnorm(n+nzeroes), a.psi2=rnorm(n+nzeroes),
       ...
       a.p0 = rnorm(n+nzeroes), a.p1=rnorm(n+nzeroes), a.p2=rnorm(n+nzeroes), a.p3=rnorm(n+nzeroes) )
  }

##----- 4 - Run the model and save the results -----

# Run the model in jags
#fit <- jags(data=jags_data, inits=inits, parameters.to.save=params, n.chains=3, n.iter=500, n.burnin=50, n.thin=10, model.file=here("src","model_foreco.txt"))

fit <- jags(data=jags_data, inits=inits, parameters.to.save=params, n.chains=3, n.iter=100000, n.burnin=50000, n.thin=100, model.file=here("src","model_foreco.txt"))


# Save results  
saveRDS(fit, here("results", "model_FORECO.rds"))
